...
// Helper: get leaf category (navega na arvore de categorias ate encontrar uma sem filhos)
async function getLeafCategory(category_id) {
        let currentId = category_id;
        for (let i = 0; i < 10; i++) {
                    const { data } = await axios.get(`${ML_BASE}/categories/${currentId}`);
                    if (!data.children_categories || data.children_categories.length === 0) {
                                    return currentId;
                    }
                    currentId = data.children_categories[0].id;
        }
        return currentId;
}
...
router.post('/anuncio', async (req, res) => {
        try {
            ...
            // Garante que a categoria e uma leaf category (sem subcategorias)
            const rawCategoryId = category_id || 'MLB1051';
                    const leafCategoryId = await getLeafCategory(rawCategoryId);
