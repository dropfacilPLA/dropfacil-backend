...
// Get CJ access token using apiKey (new method)
async function getCJToken() {
        if (cjToken && cjTokenExpiry && Date.now() < cjTokenExpiry) return cjToken;

  console.log('CJ Auth attempt - apiKey:', process.env.CJ_API_KEY ? process.env.CJ_API_KEY.substring(0, 20) + '...' : 'NOT SET');
  ...
  if (res.data.result) {
            ...
            cjTokenExpiry = Date.now() + (180 * 24 * 60 * 60 * 1000); // 180 days
          return cjToken;
  }
        throw new Error('CJ auth failed: ' + res.data.message);
}

// GET /api/cj/test - test auth
...
// POST /api/cj/frete - calculate shipping
