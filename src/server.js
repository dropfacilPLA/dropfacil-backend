// DropFacil v5.5 - frete ME2 corrigido + estoque correto
...
// ====== Buscar atributos obrigatorios e montar valores padrao ======
...
// ====== CORRIGIDO: Publicar no ML com frete ME2 correto e estoque certo ======
async function publicarNoML(titulo, preco, imagem, accessToken, categoriaSugerida, quantidade) {
    const qtd = (quantidade && quantidade > 0) ? parseInt(quantidade) : 10;

  // CORRECAO PRINCIPAL: shipping correto para dropshipping sem contato com produto
  // mode: 'me2' = Mercado Envios (obrigatorio para nao aparecer "combinar com vendedor")
  // logistic_type: 'drop_off' = vendedor leva a agencia (sem estoque fisico proprio)
  // free_shipping: false para nao prejudicar margem
  const shippingCorreto = {
        mode: 'me2',
        local_pick_up: false,
        free_shipping: false,
  ...
  const baseListagem = {
        ...
        shipping: shippingCorreto,
        description: { plain_text: titulo + '\n\nEnvio pelo Mercado Envios. Produto novo com qualidade garantida. Entrega rapida para todo o Brasil.' },
  ...
  // Categorias folha seguras para fallback
  ...
  let lastError = null;
    for (const listing of tentativas) {
          try {
                  console.log('[PUBLICAR] Tentando cat:', listing.category_id, '| qtd:', listing.available_quantity, '| attrs:', listing.attributes?.length || 0);
                  const { data } = await axios.post('https://api.mercadolibre.com/items', listing, {
                            headers: { Authorization: 'Bearer ' + accessToken, 'Content-Type': 'application/json' }
                  });
                  console.log('[PUBLICAR] Sucesso! Item:', data.id, '| Status:', data.status, '| Frete:', data.shipping?.mode);
                  return data;
          } catch (e) {
...
app.post('/api/publicar', async (req, res) => {
    try {
      ...
      console.log('[PUBLICAR] Item criado:', mlData.id, '| Permalink:', mlData.permalink);
...
// ====== NOVO: Atualizar estoque de produto publicado ======
app.post('/api/ml/atualizar-estoque', async (req, res) => {
    try {
          const { ml_item_id, quantidade } = req.body;
          if (!ml_item_id || !quantidade) return res.status(400).json({ error: 'ml_item_id e quantidade sao obrigatorios' });
          const token = await getMlToken();
          ...
          console.log('[ESTOQUE] Atualizado:', ml_item_id, '-> qtd:', quantidade);
...
app.listen(PORT, () => {
    console.log('DropFacil v5.5 porta ' + PORT);
